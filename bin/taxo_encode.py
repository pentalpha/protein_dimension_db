...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print(
            "Usage: python taxo_make_model.py <model_dir> <id_col> <taxids_col> <input_df>"
        )
        sys.exit(1)

    model_dir = sys.argv[1]
    id_col = sys.argv[2]
    taxids_col = sys.argv[3]
    ...
    if end_fmt in ["tsv", "csv"]:
        ...
        taxids = read_df[taxids_col].to_list()
    else:
        ids = read_df[id_col].tolist()
        taxids = read_df[taxids_col].tolist()

    print(f"Number of IDs: {len(ids)}")
    print(f"Number of taxid lists: {len(taxids)}")
    print(f"IDs: {ids[:10]}")
    print(f"Taxids: {taxids[:10]}")

    raw_data = [";".join([tx.strip() for tx in line.split(";")]) for line in taxids]
    ...
    print(f"Lens: {len(ids), len(embs_list), len(taxids)}")
    ...
    new_pqt_df.write_parquet("emb.taxid_autoencoded.parquet")
