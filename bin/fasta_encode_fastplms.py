...
poolings = [("mean", "std", "max"), ("parti",), ("norm",)]
poolings_all = [("mean", "std", "max"), ("parti",), ("cls", "norm")]
poolings_ankh = [("parti",), ("norm",), ("mean", "std", "max")]
poolings_e1 = [("mean", "std", "max"), ("norm",)]
...
def embed_to_cache(
    ...
    if "ankh" in model_name.lower():
        poolings_to_use = poolings_ankh
    else:
        poolings_to_use = poolings
    ...
    if full:
        ...
        embeddings = embeddings.as_dict(key='sequence')

        emb_example = next(iter(embeddings.values()))
        fully_pooled_len = emb_example.shape[0]
        actual_emb_width = fully_pooled_len
        ...
        for seq, emb in embeddings.items():
            ...
            embs["full"].append(concat_emb_np)
            seqs_embedded.append(seq)
    else:
        for current_pools in poolings_to_use:
            for p in current_pools:
                embs[p] = []
            n_poolings = len(current_pools)
            seqs_embedded = []
            ...
            for seq, emb in embeddings.items():
                if not printed:
                    ...
                    print(type(emb))
                concat_emb_np = emb.cpu().numpy()
                for pooling_name in current_pools:
...
if __name__ == "__main__":
    ...
    model = AutoModel.from_pretrained(
        ...
        #device_map='auto',
        #device=device
        force_download=True, # Forces a re-download of the weights
        resume_download=False
    ).to("cuda")

    print(f"Loading cache from {cache_path} for {model_name} ...")
    cache = EmbCache(cache_path, model_name, full=full_embeddings)
    durations = []
    non_embedded_seqs = cache.list_non_embedded(fasta_path)
    while len(non_embedded_seqs) > 0:
        print("Preparing next macro-batch...")
        next_to_embed = []
        while (
        ...
        try:
            n_tokens_next = sum([len(s) for s in next_to_embed])
            print(f"Embedding batch of {len(next_to_embed)} sequences, {n_tokens_next} tokens, into {next_parquet} and {next_txt} ...")
            duration = embed_to_cache(
                next_to_embed, model, MAX_TOKENS_PER_BATCH, 
                next_parquet, next_txt, model_name, full=full_embeddings
            )
