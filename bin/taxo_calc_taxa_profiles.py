...
import polars as pl
from bioinfo_utils.util_base import run_command
...
class TaxaProfileModel:
    ...
    def __init__(
        self, taxids_to_use: List[str], taxallnomy_df_path: str = None
    ) -> None:
    ...
    def find_missing_taxids(self, taxids):
        ...
        missing_taxids = [
            k for k, c in taxid_frequency.items() if k not in self.taxid_parent_lists
        ]
        total_missing = sum(taxid_frequency[k] for k in missing_taxids)
        freq_relative = total_missing / len(taxids)
        print(
            "Taxids missing from taxalnomy:",
            len(missing_taxids),
            "total missing:",
            total_missing,
            file=sys.stderr,
    ...
    def calc(self, taxid):
        ...
        if not taxid in self.taxid_parent_lists:
            ...
            print(
                len(self.taxid_parent_lists), "length of parent lists", file=sys.stderr
            )
            ...
            if common_taxid == taxid:
                ...
                n_equals = sum(
                    [
                        t1 == t2
                        for t1, t2 in zip(
                            taxid_parents, self.taxid_parent_lists[common_taxid]
                        )
                    ]
                )
...
if __name__ == "__main__":
    ...
    for profile_len in profile_lengths:
        if top_taxa_dir and top_taxa_dir != "None":
            ...
            tops_loadpath = path.join(
                top_taxa_dir, "top_taxa_" + str(profile_len) + ".txt"
            )
            ...
            taxids_to_use = [
                float(x)
                for x in open(tops_loadpath, "r").read().split("\n")
                if len(x) > 0
            ]
        else:
        ...
        profile_model = TaxaProfileModel(
            taxids_to_use, taxallnomy_df_path=taxallnomy_df_path
        )
        ...
        if output_suffix == "":
            save_path = (
                release_dir + "/emb.taxa_profile_" + str(profile_len) + ".parquet"
            )
        else:
            save_path = (
                release_dir
        ...
        onehot = np.asarray(
            [profile_model.calc_onehot(taxid) for taxid in tqdm(taxids)]
        )
        if output_suffix == "":
