...
class Embedder:
    def __init__(self, is_large, caches_dir) -> None:
        ...
        self.base_cache_name = (
            self.cache_dir + "/" + self.model_name + "_CACHEN_.json.gz"
        )
    ...
    def load_existing_embeddings(self, sequences):
        ...
        for seq in tqdm(sequences):
            if seq in self.cached_seqs:
                if seq not in loaded_cache:
                    ...
                    try:
                        for loaded_seq, emb in json.load(
                            gzip.open(c_path, "rt")
                        ).items():
        ...
        print(
            f"Loaded {total_loaded} embeddings from cache ({total_loaded/len(embeddings)*100:.2f}%)",
            file=sys.stderr,
        )
        if n_corrupted > 0:
    ...
    def aminos_to_embeddings(self, protein_sequences):
        ...
        with torch.no_grad():
            ...
            embeddings_np = [
                embeddings.last_hidden_state[i].mean(dim=0).numpy()
    ...
    def cache_results(self, seqs, embeddings):
        not_cached = {
            s: e.tolist() for s, e in zip(seqs, embeddings) if not s in self.cached_seqs
        }
        if len(not_cached) > 0:
            cache_i = 1
...
if __name__ == "__main__":
    ...
    for is_large in [False, True]:
        ...
        df = pl.DataFrame({"id": all_ids, "emb": all_embeddings})
