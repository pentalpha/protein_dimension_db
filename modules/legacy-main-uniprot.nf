...
process sort_trembl {
    ...
    output:
    path "trembl.txt", emit: ids
    path "trembl_sorted.fasta.gz", emit: uniprot_sorted

    script:
    """
    python ${projectDir}/src/fasta_sort.py ${original_uniprot} trembl.txt trembl_sorted.fasta.gz
    """
}
...
process filter_large_proteins {
    ...
    input:
    path sorted_swissprot
    path sorted_trembl
    val max_protein_len

    output:
    path "uniprot.fasta", emit: fasta
    path "ids.txt", emit: ids

    script:
    """
    python ${projectDir}/src/fasta_filter_by_len.py ${sorted_swissprot} ${sorted_trembl} uniprot.fasta ids.txt ${max_protein_len}
    """
}
...
process list_taxids {
    ...
    script:
    """
    python ${projectDir}/src/taxo_list_uniprot_taxids.py ${swissprot_fasta} ${trembl_fasta} ${sorted_ids} taxid.tsv
    """
}
...
process process_goa {
    ...
    script:
    """
    python ${projectDir}/src/go_filter_gaf.py ${evi_not_to_use_path} ${gocheck_do_not_annotate} ${go_basic_path} ${goa_raw_path} ${sorted_ids} rerun
    """
}
...
process list_annotated_trembl {
    ...
    output:
    path "trembl_ids.txt", emit: trembl_ids

    script:
    """
    python ${projectDir}/src/fasta_list_usable_trembl.py ${go_by_uniprot}
    """
}
...
process prottrans_embs {
    ...
    script:
    """
    python src/plm_create_prottrans_embs.py ${prot_trans_original} ${sorted_ids} emb.prottrans.parquet
    """
}
...
process taxa_profiles {
    ...
    """
    python3 src/taxo_calc_taxa_profiles.py ${taxallnomy_tsv_path} ${go_experimental_mf} ${taxids_path}
    """
}
...
process calc_ankh_embeddings {
    ...
    script:
    """
    python ${projectDir}/src/plm_ankh_calc.py ${sorted_uniprot_not_large} ${ankh_cache_path} ${all_uniprot_ids}
    """
}
...
process calc_esm_embeddings {
    ...
    script:
    """
    python ${projectDir}/src/plm_esm_calc.py ${sorted_uniprot_not_large} ${esm_cache_path} ${all_uniprot_ids}
    """
}
...
workflow {
    ...
    trembl_all_url = "https://ftp.uniprot.org/pub/databases/uniprot/knowledgebase/complete/uniprot_trembl.fasta.gz"
    trembl_test_url = "https://ucrania.imd.ufrn.br/~pitagoras/protein_dimension_db/test_input/uniprot_trembl.fasta.gz"

    goa_url = Channel.value(goa_all_url)
    uniprot_url = Channel.value(uniprot_all_url)
    trembl_url = Channel.value(trembl_all_url)
    if (params.mode == 'test') {
        goa_url = Channel.value(goa_test_url)
        uniprot_url = Channel.value(uniprot_test_url)
        trembl_url = Channel.value(trembl_test_url)
    }

    swissprot_path = download_uniprot(uniprot_url)
    trembl_path = download_trembl(trembl_url)
    ...
    sort_swissprot(swissprot_path)
    process_goa(
        sort_swissprot.out.ids,
        goa_raw_path,
        gocheck_do_not_annotate,
        params.evi_not_use_path,
        go_basic_path,
    )
    index_go_by_term(process_goa.out.go_by_uniprot)
    list_annotated_trembl(process_goa.out.go_by_uniprot)
    sort_trembl(trembl_path)

    filter_large_proteins(
        sort_swissprot.out.uniprot_sorted,
        sort_trembl.out.uniprot_sorted,
        params.max_protein_len,
    )
    taxids = list_taxids(sort_swissprot.out.uniprot_sorted, sort_trembl.out.uniprot_sorted, filter_large_proteins.out.ids)
